...
extern void send_email_with_attachment(const string& to, const string& subject, const string& body, const string& filepath);
...
void send_picture() {
    ...
    string image_path = temp_dir + "captured_photo.jpg";
    ...
    if (frame.empty()) {
        cerr << "Failed to capture image after " << max_attempts << " attempts!" << endl;
        return;
    }

    // Save image to file with error checking
    try {
        bool saved = cv::imwrite(image_path, frame);
        if (!saved) {
            cerr << "Failed to save image to: " << image_path << endl;
            return;
        }

        // Verify file was actually created and has content
        if (!std::filesystem::exists(image_path) || std::filesystem::file_size(image_path) == 0) {
            cerr << "Image file was not created properly!" << endl;
            return;
        }
...
void start_recording() {
    ...
    recordingThread = std::thread([]() {
        ...
        try {
            ...
            if (!writer.isOpened()) {
                cerr << "Failed to open VideoWriter for: " << current_video_path << endl;
                recording = false;
                return;
            }
            ...
            while (recording) {
                ...
                if (frame.empty()) {
                    ...
                    if (consecutive_empty_frames >= max_empty_frames) {
                        cerr << "Too many consecutive empty frames, stopping recording!" << endl;
                        break;
                    }
                ...
                auto target_time = start_time + std::chrono::milliseconds(frame_count * 1000 / fps);
...
void stop_and_send_recording() {
    ...
    try {
        ...
        cout << "Video file created successfully (" << file_size << " bytes)" << endl;
