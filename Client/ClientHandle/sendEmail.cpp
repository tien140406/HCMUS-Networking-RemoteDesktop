...
string getMimeType(const string &extension)
{
    static const std::map<string, string> mimeTypes = {
        {".jpg", "image/jpeg"}, {".jpeg", "image/jpeg"}, {".png", "image/png"}, {".txt", "text/plain"}, {".avi", "video/x-msvideo"}, {".mp4", "video/mp4"}, {".mov", "video/quicktime"}};
                                ...
                                const std::string &filepath)
{
    ...
    // Set proper email headers using CURLOPT_HTTPHEADER
    std::string newSubject = "[RemoteDesktop] " + subject;
    std::string toHeader = "To: " + toEmail;
    std::string fromHeader = "From: " + gmail_username;
    std::string subjectHeader = "Subject: " + newSubject;
    ...
    headers = curl_slist_append(headers, mimeHeader.c_str());

    curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);

    // Add body part (no need for separate headers part now)
    ...
    std::cout << "[Email] Sending email with subject: " << subject << std::endl;
    ...
    curl_slist_free_all(headers); // Don't forget to free headers
